...
local function on_rightclick_callback(pos, node, clicker, itemstack, pointed_thing)
    ...
    if clicker:is_player() then
        local result = Industria.controllers.isValidController(pos, node);
        ...
        if result.data == nil then
            ...
            Industria.formspecs:showInputControllerID(clicker:get_player_name(), pos);
            return;
        end

        local unit = result.data;
        --Se l'unità non è valida o non ha l'id allora esci
        if unit == nil or unit.controller_id == nil then
            return;
        end
        ...
        if unit.protected and unit.owner ~= clicker:get_player_name() then
            ...
            Industria.formspecs:showControllerMainForm(clicker:get_player_name(),
                Industria.controllers.toControllerCode(unit.controller_id, unit.owner) or "");
        end
    end
end
...
local function after_dig_callback(pos, oldnode, oldmetadata, digger)
    ...
    if not oldmetadata.fields["controller_id"] or not oldmetadata.fields["controller_owner"] then
        --Se non lo ho non devo rimuovere nulla
        return;
    end
    local plcid = oldmetadata.fields["controller_id"];       --prendo l'id
    local plcowner = oldmetadata.fields["controller_owner"]; --prendo l'owner

    local res = Industria.controllers:removeController(plcid, plcowner);
    if digger:is_player() then
        if res.completed then
            core.chat_send_player(digger:get_player_name(), "Controller removed");
        else
...
core.register_node("industria:basecontroller", {
    description = "Base Controller",
    drawtype = "mesh",
    mesh = "industria_basecontroller.glb",
    tiles = { "industria_base_controller.png" },
    node_box = {
        type = "fixed",
        fixed = {
            { -1 / 16, -4.5 / 16, 8 / 16,
